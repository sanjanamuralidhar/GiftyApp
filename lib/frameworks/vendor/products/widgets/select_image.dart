...
class SelectImage extends StatelessWidget {
  ...
  Future _getImageFromCamera() async {
    ...
    if (image != null) {
      ...
      img.Image _image = img.decodeImage(image.readAsBytesSync());
      if (isIos) {
  ...
  Future _getImageFromGallery() async {
    ...
    if (image != null) {
      if (isIos) {
        ...
        /// Remove below rotation fix. Reference: https://github.com/inspireui/support/issues/4798#issuecomment-683730395
        /// _image = img.copyRotate(_image, 90);
        path = path.substring(0, path.lastIndexOf('.')) + '.png';
        await image.rename(path).then((onValue) {
          onValue.writeAsBytesSync(img.encodePng(_image));
