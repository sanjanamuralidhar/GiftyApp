...
class BlogModel with ChangeNotifier {
  ...
  Future getBlogs() async {
    ...
    switch (Config().type) {
      ...
      //   // print("Blog length: ${_blogs.length}");
      //   break;
      default:
        _cursor ??= 1;
        final data = await Blog.getBlogs(
  ...
  Future refresh() async {
    ...
    _blogs = null;
    notifyListeners();
    return Future.delayed(const Duration(milliseconds: 3000), getBlogs);
  }
}
...
class Blog {
  ...
  static Future getBlogs({url, categories, page = 1}) async {
    try {
