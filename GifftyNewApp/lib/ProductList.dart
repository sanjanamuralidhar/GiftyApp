...
import 'dart:math';
//****** @removeFromcart aneesh bloc ****///
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:giftty/Helper/AppBtn.dart';
import 'package:giftty/Helper/SimBtn.dart';
...
class StateProduct extends State<ProductList> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // productListBloc.dispatch(FetchNotificationEvent());
  ...
  Widget _removeFromCart(BuildContext context, int index) {
    setState(() {
      productListBloc.add(
        OnRemoveFromCartEvent(
          userId: CUR_USERID,
          productvarientId: productList[index]
    ...
    return Container(
      child: BlocListener<ProductListBloc, ProductListState>(
        listener: (context, state) {
          if (state is RemovingFromCartFailed) {
        ...
        child: BlocBuilder<ProductListBloc, ProductListState>(
          builder: (context, state) {
            if (state is ProductListInitialState) {
              ...
              getProduct("0");
            } else if (state is RemovingFromCartFailed) {
  ...
  removeFromCart(int index) async {
    print('remove fronm caryheih dfkjbhv cdjb 123456');
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      if (CUR_USERID != null)
        try {
