...
class SectionList extends StatefulWidget {
  ...
  SectionList({Key key, this.index, this.sectionModel, this.updateHome})
      : super(key: key);
...
class StateSection extends State<SectionList> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    widget.sectionModel.offset = widget.sectionModel.productList.length;

    widget.sectionModel.selectedId = [];

    widget.sectionModel.productList.clear();
    getSection("0");
  ...
  Future<Null> _refresh() {
    if (mounted)
      setState(() {
        _isLoading = true;
        isLoadingmore = true;
        widget.sectionModel.offset = 0;
        widget.sectionModel.totalItem = 0;
        widget.sectionModel.selectedId = [];
        selId = '';
      });
              ...
              child: Card(
                  ...
                  child: Material(
                    ...
                    child: InkWell(
                        ...
                        onTap: () {
                          widget.sectionModel.productList.length != 0
                              ? setState(() {
                                  listType = !listType;
                                })
                      ...
                      children: <Widget>[
                        listType
                            ? ListView.builder(
                                ...
                                itemCount: (widget.sectionModel.offset <
                                        widget.sectionModel.totalItem)
                                    ? widget.sectionModel.productList.length +
                                        1
                                    : widget.sectionModel.productList.length,
                                physics: AlwaysScrollableScrollPhysics(),
                                itemBuilder: (context, index) {
                                  return (index ==
                                              widget.sectionModel.productList
                                                  .length &&
                                          isLoadingmore)
                                      ? Center(
  ...
  Widget listItem(int index) {
    Product model = widget.sectionModel.productList[index];

    double price = double.parse(widget.sectionModel.productList[index]
        .prVarientList[model.selVarient].disPrice);
    if (price == 0)
      price = double.parse(widget.sectionModel.productList[index]
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Stack(children: <Widget>[
          widget.sectionModel.productList[index].availability == "0"
          ...
          Padding(
            ...
            child: Row(
              children: <Widget>[
                Hero(
                  tag: "$index${widget.sectionModel.productList[index].id}",
                  child: ClipRRect(
                      borderRadius: BorderRadius.circular(7.0),
                      child: FadeInImage(
                        image: NetworkImage(
                            widget.sectionModel.productList[index].image),
                        height: 80.0,
                        width: 80.0,
                      ...
                      children: <Widget>[
                        Row(
                          children: [
                            Expanded(
                              child: Text(
                                widget.sectionModel.productList[index].name,
                                style: Theme.of(context)
                                    .textTheme
                                    .subtitle2
                            ...
                            widget.sectionModel.productList[index].isFavLoading
                                ? Container(
                                    height: 15,
                                    width: 15,
                                ...
                                : InkWell(
                                    child: Padding(
                                      padding: const EdgeInsets.symmetric(
                                          horizontal: 8.0, vertical: 3),
                                      child: Icon(
                                        widget.sectionModel.productList[index]
                                                    .isFav ==
                                                "0"
                    ...
                    Container(
                      ...
                      child: InkWell(
                          ...
                          onTap: () {
                            if (mounted)
                              setState(() {
                                widget.sectionModel.selectedId.clear();
                              });
                          }),
                    ),
                  ],
                        ...
                        child: Row(
                          ...
                          Expanded(
                              ...
                              child: Container(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemCount:
                                        widget.sectionModel.filterList.length,
                                    itemBuilder: (context, index) {
                                      attsubList = widget.sectionModel
                                          .filterList[index].attributeValues
                                          .split(',');

                                      attListId = widget.sectionModel
                                          .filterList[index].attributeValId
                                          .split(',');

                                      if (filter == "") {
                                        filter = widget
                                            .sectionModel.filterList[0].name;
                                      }
                          ...
                          Expanded(
                              ...
                              child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    if (filter ==
                                        widget.sectionModel.filterList[index]
                                            .name) {
                                      attsubList = widget.sectionModel
                                          .filterList[index].attributeValues
                                          .split(',');
              ...
              child: Row(children: <Widget>[
                Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(widget.sectionModel.totalItem.toString()),
                ...
                SimBtn(
                  ...
                  onBtnSelected: () {
                    if (widget.sectionModel.selectedId != null) {
                      selId = widget.sectionModel.selectedId.join(',');
                      clearList("0");
      ...
      if (this.mounted) {
        if (mounted)
          setState(() {
            ...
            if (widget.sectionModel.offset < widget.sectionModel.totalItem)
              getSection("0");
          });
      }
    }
    ...
    if (_isNetworkAvail) {
      try {
        var parameter = {
          ...
          PRODUCT_OFFSET: widget.sectionModel.productList.length.toString(),
          SEC_ID: widget.sectionModel.id,
          TOP_RETAED: top,
          PSORT: sortBy,
          PORDER: orderBy,
        };
        ...
        if (!error) {
          ...
          offset = widget.sectionModel.productList.length;
          ...
          if (offset < total) {
            ...
            offset = widget.sectionModel.offset + perPage;

            widget.sectionModel.offset = offset;
            widget.sectionModel.totalItem = total;
          }
        } else {
          isLoadingmore = false;
          setSnackbar(msg);
  ...
  _setFav(int index) async {
    ...
    if (_isNetworkAvail) {
      try {
        if (mounted)
          setState(() {
            widget.sectionModel.productList[index].isFavLoading = true;
          });

        var parameter = {
          USER_ID: CUR_USERID,
          PRODUCT_ID: widget.sectionModel.productList[index].id
        };
        Response response =
        ...
        String msg = getdata["message"];
        if (!error) {
          widget.sectionModel.productList[index].isFav = "1";
          widget.updateHome();
        } else {
          setSnackbar(msg);
        }
        ...
        if (mounted)
          setState(() {
            widget.sectionModel.productList[index].isFavLoading = false;
          });
      } on TimeoutException catch (_) {
        setSnackbar(getTranslated(context, 'somethingMSg'));
      }
  ...
  _removeFav(int index) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        String msg = getdata["message"];
        if (!error) {
          widget.sectionModel.productList[index].isFav = "0";

          favList.removeWhere((item) =>
              item.productList[0].prVarientList[0].id ==
              widget.sectionModel.productList[index].prVarientList[0].id);
