...
bool isProgress = false, _isFavLoading = true;
int offset = 0;
int total = 0;
...
class StateFav extends State<Favorite> with TickerProviderStateMixin {
  ...
  FavoriteCrudBloc favoriteCrudBloc;
  FavBloc sampleBloc;

  @override
  void initState() {
    super.initState();
    favoriteCrudBloc = BlocProvider.of<FavoriteCrudBloc>(context);
    favoriteBloc = BlocProvider.of<FavoriteBloc>(context);
    favoriteBloc.add(FetchFavoriteEvent());
                  ...
                  child: BlocBuilder<FavoriteBloc, FavoriteState>(
                    builder: (context, state) {
                      if (state is FavoriteInitialState) {
                        ...
                        if (CUR_USERID != null) {
                          favList = state.articles;
                          print(
                              'ooopoooooooodffoooooooooooooooo  ${favList.length}');
                        ...
                        print(
                            'gyug uygfguygyhghhhhhhhhhhhhhhhhhhhhhhh${state.articles.length}');
                              ...
                              Container(
                                ...
                                child: PopupMenuButton(
                                  ...
                                  onSelected: (result) async {
                                    if (result == 0) {
                                      removeFav(index);
                                      // _buildRemove(context, index);
                                    }
                                    if (result == 1) {
                                      // addToCart(index);
                                      _buildAddToCart(context, index);
                                    }
                                    if (result == 2) {
                                      if (mounted)
                                        setState(() {
  ...
  void deleteFavorite(int index) {
    favoriteCrudBloc.add(DeleteFavoriteEvent(
      id: favList[index].productId,
    ));
...
//////////////////////////// Add to cart //////////////////////
  ...
  Widget _buildAddToCart(BuildContext context, int index) {
    print('77777799000$CUR_USERID......${favList[index].productId}.....$index');
    setState(() {
      sampleBloc.add(
        OnAddToCart(
          userid: CUR_USERID,
          productVarientId: favList[index].productList[0].prVarientList[0].id,
          qty: (int.parse(favList[index]
                      .productList[0]
                      .prVarientList[0]
                      .cartCount) +
      ...
      print(
          '///////////77777799000$CUR_USERID......${favList[index].productId}.....$index');
    });
    return Container(
        ...
        child: BlocBuilder<FavBloc, FavState>(
          builder: (context, state) {
            if (state is FavInitialState) {
              print('cjdcbgdbc');
            } else if (state is FavAddingToCart) {
              print('cjdcbgdbc');
            } else if (state is FavAddedToCart) {
  ...
  removeFav(int index) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print(
            '.........userid:$CUR_USERID,.........productid: ${favList[index].productId}......$index');
        Response response =
...
///////////////////////////////////////////// BuildRemove Favorite /////////////
  // ignore: unused_element
  Widget _buildRemove(BuildContext context, int index) {
    print('77777799000$CUR_USERID......${favList[index].productId}.....$index');
    setState(() {
      sampleBloc.add(
        RemoveFav(
          userid: CUR_USERID,
          productId: favList[index].productId,
          index: index,
        ),
      );
      print(
          '////////////7777799000$CUR_USERID......${favList[index].productId}.....$index');
    });
    return Container(
      child: BlocListener<FavBloc, FavState>(
        listener: (context, state) {
          if (state is FavRemoveFailed) {
            Scaffold.of(context).showSnackBar(
              SnackBar(
                content: Text(state.message),
              ),
        ...
        child: BlocBuilder<FavBloc, FavState>(
          builder: (context, state) {
            if (state is FavInitialState) {
              print('cjdcbgdbc');
            } else if (state is FavRemovingFav) {
              print('cjdcbgdbc');
            } else if (state is FavRemoveFav) {
              _getFav();
            } else if (state is FavRemoveFailed) {
              return Container(child: Text(state.message));
            }
            return Container();
          },
  ...
  removeFavorite(int index, List<Section_Model> favList) {
    return favList.removeWhere((item) =>
        item.productList[0].prVarientList[0].id ==
        favList[index].productList[0].prVarientList[0].id);
  }
  ...
  showContent(List<Section_Model> favList) {
    print('vcyhsvcjhvd sghdvc,,,,,,,,,$favList');
